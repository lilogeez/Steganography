...
class Steganography:
    ...
    def merge(img1, img2):
        ...
        # Dapatkan peta piksel dari dua gambar
        pixel_map1 = img1.load()
        pixel_map2 = img2.load()

        # Buat gambar baru yang akan dikeluarkan
        ...
        for i in range(img1.size[0]):
            for j in range(img1.size[1]):
                ...
                # Gunakan piksel hitam sebagai default
                rgb2 = Steganography.__int_to_bin((0, 0, 0))

                # Periksa apakah posisi peta piksel valid untuk gambar kedua
                if i < img2.size[0] and j < img2.size[1]:
                    rgb2 = Steganography.__int_to_bin(pixel_map2[i, j])

                # Gabungkan dua piksel dan ubah menjadi tupel bilangan bulat
    ...
    def unmerge(img):
        ...
        # Buat gambar baru dan muat peta piksel
        new_image = Image.new(img.mode, img.size)
        pixels_new = new_image.load()

        # Tuple digunakan untuk menyimpan gambar ukuran asli
        original_size = img.size

        for i in range(img.size[0]):
            for j in range(img.size[1]):
                # Dapatkan RGB (sebagai tupel string) dari piksel saat ini
                r, g, b = Steganography.__int_to_bin(pixel_map[i, j])

                # Ekstrak 4 bit terakhir (sesuai dengan gambar tersembunyi)
                # Menggabungkan 4 bit nol karena kami bekerja dengan 8 bit
                rgb = (r[4:] + '0000',
                       g[4:] + '0000',
                       b[4:] + '0000')

                # Ubah menjadi tupel bilangan bulat
                pixels_new[i, j] = Steganography.__bin_to_int(rgb)

                # Jika ini adalah posisi yang 'valid', simpanlah
                # sebagai posisi valid terakhir
                if pixels_new[i, j] != (0, 0, 0):
                    original_size = (i + 1, j + 1)

        # Pangkas gambar berdasarkan piksel 'valid'
